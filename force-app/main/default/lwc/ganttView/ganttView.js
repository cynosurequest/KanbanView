...
export default class GanttView extends LightningElement {
  ...
  /*** /Navigation ***/
  tasks = [
    { id: 1, name: 'Task 1', startDate: '2023-09-13', duration: 5 },
    { id: 2, name: 'Task 2', startDate: '2023-09-18', duration: 3 ,},
];
...
generateGanttChart() {
    const chartContainer = this.template.querySelector('.gantt-chart');
    this.tasks.forEach(task => {
        const taskElement = document.createElement('div');
        taskElement.classList.add('gantt-task');
        taskElement.style.width = `${task.duration * 20}px`;
        taskElement.style.marginLeft = `${this.daysBetween(new Date(), new Date(task.startDate)) * 20}px`;
        taskElement.innerHTML = `${task.name} - ${task.startDate} - ${this.addDays(task.startDate, task.duration)}`;
