...
export default class GanttChartResource extends LightningElement {
  ...
  setProjects() {
    ...
    Object.keys(self._resource.allocationsByProject).forEach(projectId => {
      ...
      // });
      self.resource.allocationsByProject[projectId].map(function(allocation) {
        console.log('Object.keys :' + Object.keys(allocation));
        let alloc = {};
        alloc.class = self.calcClass(allocation);
        alloc.style = self.calcStyle(allocation);
        alloc.labelStyle = self.calcLabelStyle(allocation);
        console.log('alloc :' + JSON.stringify(alloc));
        const newAllocation = {...allocation, ...alloc};
        project.allocations.push(newAllocation);
      });
